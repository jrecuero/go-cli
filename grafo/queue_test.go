...
func TestQueue_Queue(t *testing.T) {
	...
	if err := system.AddQueue(nil, grafo.ServerToVertex(root), 0); err != nil {
		t.Errorf("Queue:AddQueue: return error code: %#v\n", err)
	}
	if err := system.AddQueue(grafo.ServerToVertex(root), grafo.ServerToVertex(queue1), 10); err != nil {
		t.Errorf("Queue:AddQueue: return error code: %#v\n", err)
	}
	//tools.ToDisplay("%s\n", system.ToMermaid())
}
...
func TestQueue_Check(t *testing.T) {
	...
	root := grafo.NewServer("root/1", nil)
	queue1 := grafo.NewServer("queue/1", nil)
	system.AddQueue(nil, grafo.ServerToVertex(root), 0)
	system.AddQueue(grafo.ServerToVertex(root), grafo.ServerToVertex(queue1), 10)
	b := root.Edges[0]
	b.Check(grafo.NewJob("job/1", 1))
	b.Check(grafo.NewJob("job/2", 2))
	if len(root.Edges[0].(*grafo.Queue).Jobs) != 2 {
		t.Errorf("Queue:Check: edges len mismatch: exp: %d got: %d\n", 2, len(root.Edges[0].(*grafo.Queue).Jobs))
	}
	//tools.ToDisplay("%#v\n", root.Edges[0].(*grafo.Queue).Jobs)
	//for _, j := range root.Edges[0].(*grafo.Queue).Jobs {
	//    tools.ToDisplay("%#v\n", j)
	//}
}
...
func TestQueue_Serve(t *testing.T) {
	...
	//    tools.ToDisplay("%#v\n", j)
	//}
	jobs := &root.Edges[0].(*grafo.Queue).Jobs
